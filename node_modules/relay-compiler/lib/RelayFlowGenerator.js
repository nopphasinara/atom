...
var _require4 = require('graphql-compiler'),
...
function selectionsToBabel(selections, state, refTypeName) {
  ...
  return unionTypeAnnotation(types.map(function (props) {
    if (refTypeName) {
      props.push(readOnlyObjectTypeProperty('$refType', require('@babel/types').genericTypeAnnotation(require('@babel/types').identifier(refTypeName))));
    }
  ...
  return {
    leave: {
      Root: function Root(node) {
        ...
        return require('@babel/types').program([].concat((0, _toConsumableArray3['default'])(getFragmentImports(state)), (0, _toConsumableArray3['default'])(getEnumDefinitions(state)), (0, _toConsumableArray3['default'])(inputObjectTypes), [inputVariablesType, responseType]));
      },
      Fragment: function Fragment(node) {
        ...
        var refType = require('@babel/types').declareExportDeclaration(require('@babel/types').declareOpaqueType(require('@babel/types').identifier(refTypeName), null, require('@babel/types').genericTypeAnnotation(require('@babel/types').identifier('FragmentReference'))));
...
function generateInputObjectTypes(state) {
  return Object.keys(state.generatedInputObjectTypes).map(function (typeIdentifier) {
    ...
    require('fbjs/lib/invariant')(typeof inputObjectType !== 'string', 'RelayCompilerFlowGenerator: Expected input object type to have been' + ' defined before calling `generateInputObjectTypes`');
...
function groupRefs(props) {
  ...
  if (refs.length > 0) {
    var _value3 = intersectionTypeAnnotation(refs.map(function (ref) {
      return require('@babel/types').genericTypeAnnotation(require('@babel/types').identifier(getRefTypeName(ref)));
    }));
    result.push({
      key: '$fragmentRefs',
      conditional: false,
