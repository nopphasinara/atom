...
class ThreadTreeNode extends React.Component {
  ...
  componentDidMount() {
    ...
    }), this._selectTrigger.asObservable().let((0, _observable().fastDebounce)(100)).switchMap(() => this._getFrames(true)).subscribe(frames => {
      this.setState({
        childItems: frames
      });
    }), (0, _event().observableFromSubscribeFunction)(model.onDidChangeCallStack.bind(model)).let((0, _observable().fastDebounce)(100)).startWith(null).switchMap(() => this._getFrames().switchMap(frames => {
