...
function _showTriggerConflictWarning() {
  const data = _interopRequireDefault(require("./showTriggerConflictWarning"));

  _showTriggerConflictWarning = function () {
    return data;
  };
...
class HyperclickForTextEditor {
  ...
  constructor(textEditor, hyperclick) {
    ...
    this._lastMouseEvent = null; // Cache the most recent suggestion so we can avoid unnecessary fetching.
    ...
    this._subscriptions.add(_featureConfig().default.observe(process.platform === 'darwin' ? 'hyperclick.darwinTriggerKeys' : process.platform === 'win32' ? 'hyperclick.win32TriggerKeys' : 'hyperclick.linuxTriggerKeys', newValue_ => {
      const newValue = newValue_; // For all Flow knows, newValue.split could return any old strings
  ...
  _setupMouseListeners() {
    const addMouseListeners = () => {
      ...
      const removalDisposable = new (_UniversalDisposable().default)(() => {
  ...
  _onMouseMove(event) {
    const mouseEvent = event; // We save the last `MouseEvent` so the user can trigger Hyperclick by
  ...
  _onKeyDown(event) {
    const mouseEvent = event; // Show the suggestion at the last known mouse position.
  ...
  _observeSuggestions() {
    return this._fetchStream.map(mouseEvent => {
      ...
      if (position == null) {
        ...
        const match = (0, _range().wordAtPosition)(this._textEditor, position);
      ...
      return _RxMin.Observable.using(() => this._showLoading(), () => _RxMin.Observable.defer(() => this._hyperclick.getSuggestion(this._textEditor, position)).startWith(null) // Clear the previous suggestion immediately.
      .catch(e => {
  ...
  _getMousePositionAsBufferPosition(mouseEvent) {
    ...
    try {
      ...
      (0, _log4js().getLogger)('hyperclick').error('Hyperclick: Error getting buffer position for screen position:', error);
  ...
  _isInLastSuggestion(position) {
    ...
    return (0, _range2().isPositionInRange)(position, range);
  }
  ...
  _isInLastWordRange(position) {
    ...
    return (0, _range2().isPositionInRange)(position, lastWordRange);
  }
