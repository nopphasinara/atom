...
const isJavascript = (textEditor: TextEditor) => {
  ...
  return scopeName === "source.js" || scopeName === "source.js.jsx" || scopeName === "javascript"
}
...
function makeProvider(subscriptions) {
  ...
  function buildResult(textEditor, range, suggestion, isAutomaticJump = true) {
    // console.log('buildResult.isJavascript(textEditor)', isJavascript(textEditor))
  ...
  return {
    ...
    getSuggestionForWord(
      ...
      // console.log('isJavascript(textEditor)', isJavascript(textEditor))
      if (isJavascript(textEditor)) {
        const info = cache.get(textEditor)
        // console.log('info', info)
        // console.log('parseError', info.parseError)
...
module.exports = {
  config: {
    ...
    webpack: {
      type: "boolean",
      default: true,
      title: "Resolve with Webpack",
      description: `
            Use data from Webpack to add extra module search paths. Currently
            limited to Webpack Alias support.
            `.trim(),
    },
    webpackConfigFilename: {
      ...
      description: `
            When "Resolve with Webpack" is enabled, this setting specifies the
            config file to use.
            `.trim(),
    },
  },
  ...
  getProvider() {
    const v = makeProvider(this.subscriptions)
    // console.log('subscriptions', this.subscriptions)
    // console.log('v', v)
    // v.grammarScopes = ['source.js', 'text.html.vue'] // JavaScript files
    return v
  },
  deactivate() {
